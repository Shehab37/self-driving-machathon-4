...
class RemoteAPIClient:
    ...
    def __init__(self, host="localhost", port=23000, cntport=None, *, verbose=None):
        ...
        self.verbose = (
            int(os.environ.get("VERBOSE", "0")) if verbose is None else verbose
        )
    ...
    def _waitForSignal(self, sigName):
        ...
        while True:
            retVal = (
                self.sim.getInt32Signal(sigName) != None
                or self.sim.getFloatSignal(sigName) != None
                or self.sim.getDoubleSignal(sigName) != None
                or self.sim.getStringSignal(sigName) != None
            )
            if retVal:
                break
            self.step()
        ...
        if metric:
            ...
            if distance > 0.000001:
                ...
                while result == 0:
                    ...
                    if dt == 0:
                        dt = self.sim.getSimulationTimeStep()
                    result, newPosVelAccel, syncTime = self.sim.ruckigStep(
                        ruckigObject, dt
                    )
                    if result >= 0:
                        if result == 0:
                        ...
                        outMatrix = self.sim.interpolateMatrices(
                            currentMatrix, targetMatrix, t
                        )
                        nv = [newPosVelAccel[1]]
                        na = [newPosVelAccel[2]]
                        ...
                        raise RuntimeError(
                            "sim.ruckigStep returned error code " + result
                        )
                    if result == 0:
                        self.step()
                self.sim.ruckigRemove(ruckigObject)
